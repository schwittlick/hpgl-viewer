...
| `ES s,l` | Extra space between characters and lines, as fractions of the character cell width and of the line spacing (negative tightens; no parameters → 0,0) |
| `CP s,l` | Move the pen s character cells and l lines without drawing (no parameters → CR + LF) |
| `IN` / `DF` | Reset the labelling state (size, origin, direction, slant, extra space, terminator) |

The stick font is fixed-width, like a real plotter's: every character is drawn
in the same box and advances one character cell, which is 1.5 × the `SI` width
plus whatever `ES` adds. `SI` sizes the character itself, not the cell, so the
remaining half-width is the gap between characters.

Inside a label, `LF` starts a new line (baselines two character heights apart,
scaled by `ES`) and `CR` returns to the start of the current line; other
control characters are skipped. Unsupported labelling commands (`SR`, `DR`,
`CS`/`SS`/`SA`) are ignored, so text using them still renders at the last
absolute size and direction.
...
- Text rendering: `LB` labels are drawn with a fixed-width single-stroke stick font, sized/positioned/rotated/spaced per `SI`, `LO`, `DI`, `SL` and `ES`, so plotter text (titles, calendar dates, annotations) shows up in the viewport instead of being skipped. Labels become ordinary strokes, so they are fixed, merged and exported like any other geometry (an exported `_fixed.hpgl` contains the text as polylines, not `LB` commands)
...
- Label font: `src/hpgl_font.cpp` is generated by `tools/gen_hpgl_font.py` from
  `tools/stick_font.json`, a vendored copy of the Hershey-derived fixed-width
  stick font (by Alex Forencich, with additions by Marcel Schwittlick) that the
  `cursor` project renders HP-GL text with, so both render a label identically.
  It stands in for the plotter's built-in stick font: the metrics match, and
  glyph shapes are close but not identical to what an HP plotter draws. The
  JSON is one line per glyph, keyed by character, holding the polylines the pen
  draws on a 4 × 8 design grid — x maps to the `SI` character width, y to the
  `SI` height. HPGL is a byte stream, so a glyph is looked up by its code point
  and the range above 126 follows HP's Roman-8 encoding, not Latin-1. Both the
  font and the generated table are checked in — building never needs either,
  and regenerating needs only Python:

  ```bash
  python3 tools/gen_hpgl_font.py > src/hpgl_font.cpp
  ```

  Because nothing in the build re-runs the generator, editing the JSON without
  regenerating would leave the viewer drawing the old glyphs. The `font
  generated` test regenerates the table and diffs it against the checked-in
  file, so that drift fails the suite instead of going unnoticed. It is skipped
